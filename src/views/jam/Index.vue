            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-body">
                        DATA JAM
                        <hr>
                        <router-link :to="{name: 'jam.create'}" class="btn btn-md btn-success">TAMBAH JAM</router-link>
                        ...
                        <table class="table table-striped table-bordered mt-4">
                            ...
                            <tbody>
                                ...
                                <tr v-for="(jam, index) in jams" :key="index">
                                    <td>{{ jam.range_jam }}</td>
                                    <td>{{ jam.aktif }}</td>
                                    <td class="text-center">
                                        <router-link :to="{name: 'jam.edit', params:{kode: jam.kode }}" class="btn btn-sm btn-primary mr-1">EDIT</router-link>
                                        <button @click.prevent="jamDelete(jam.kode)" class="btn btn-sm btn-danger ml-1">DELETE</button>
                                    </td>
                                </tr>
                            </tbody>
                        </table> 
...
export default {
    ...
    setup() {
        ...
        //method delete
            function jamDelete(kode) {
            
            //delete data jam by ID
            axios.delete(`http://localhost:8000/api/jam/${kode}`)
            .then(() => {
              
              //splice jams 
              jams.value.splice(jams.value.indexOf(kode), 1);
